...
for (r in 1:R) {
  ...
  Rxobs[r] <- Rgrades3$Prop[grades3$Cohort=='UG'] - Rgrades3$Prop[grades3$Cohort=='PG']
...
The $p$-value of the test is 0.0284, which is less than 0.05, given a confidence level of 95%.
...
post_fit <- fitdistr(p, "beta", start = list(shape1 = 1, shape2 = 1))
post_fit %>% tidy()

under_fit <- fitdistr(u, "beta", start = list(shape1 = 1, shape2 = 1))
under_fit %>% tidy()
```

```{r}
qbeta(c(0.025, 0.975), 9.501, 3.598) #postgrad
qbeta(c(0.025, 0.975), 5.332, 2.468) #undergrad
```

There is a 95% probability that the true proportion of postgraduate students who passed falls within the range of 0.4656 to 0.9204.

There is a 95% probability that the true proportion of undergraduate students who passed falls within the range of 0.3442 to 0.9352.
