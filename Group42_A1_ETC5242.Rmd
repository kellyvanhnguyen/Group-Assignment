...
**Assessing the fit of normal distribution using Bootstrap QQplot**
...
**Assessing the fit of beta distribution using Bootstrap QQplot**
...
p_MLEboot.mean
```
The parametric bootstrap confidence interval (CI) for the mean is [0.628, 0.724].
...
We are 99% confident that the population value $\mu$ is contained within our 99% confidence interval of [0.628, 0.724].

Since $\mu$ lies between 0.628 and 0.724, it does cover the average mark of 70% and meets the lecturer's goal.
...
- From the beta distribution, there are `r round(within_15_prct*100, 2)`% of students within 15% of the average.
- Assuming that a "Pass" is above 60%, then the proportion of students that failed the quiz is `r round(failed*100,2)`%, which is `r floor(failed*193)` students out of 193.
- Assuming that a "HD" is above 80%, then the proportion of students that got HD is `r round(hd*100,2)`%, which is `r floor(hd*193)` students out of 193.


### Overall, do you think that the quiz achieved the lecturer’s aims? [4 marks]

Overall, we would say that the quiz achieved the lecturer's aims. The average mark fell around 70%, which is within the range that was expected for the lecturer. 

Additionally, 48.65% of students scored within 15% of the MLE mean of the grades, which suggests that the quiz was fair and that students were able to demonstrate their understanding of the material. 

However, the benchmark of 60% could be considered quite high, as 25.83% of the class failed. This suggests that the quiz may have been too difficult for some students, and that the lecturer may want to reconsider the benchmark in the future.
...
As can be seen from the table above, there are more students enrolled in the undergraduate cohort (145 compared to 48), with a higher number of students passing and failing than in the postgraduate cohort (102 compared to 41). 

However, there is a higher proportion of students who passed in the postgraduate cohort compared to the undergraduate cohort, with 85.4% passing compared to 70.3%. This is a difference of 0.151. 

Overall this means that out of 48 PG students, 41 students passed, whilst 7 students failed. Out of 145 UG students, 102 students passed, whilst 43 students failed.
...
A large difference in proportions may not necessarily result in a $p$-value close to 0 due to factors such as the sample size and the alpha level.

The PG sample size is 48, whilst the UG sample size is 145. There is almost a hundred observations of difference in the sample size. Since the p-value is influenced by the size of the sample, if we have small sample size, we have less data points to estimate population parameters accurately, which lead to a larger standard error (a measure of the variability of the sample). In turn, providing a lower test statistic. A lower test statistic contributes to a smaller p-value. 
...
### This has been calculated using beta binomial function to show Posterior Dist. of PG Students
...
### This has been calculated using beta binomial function to show Posterior Dist. of UG Students 
...
Estimator: `r E_PG`
The squared error loss measures the discrepancy between the parameter and the estimate.
Based on the above estimate of the posterior expected squared loss, we can say that 98.6% of the proportion of the postgraduate students passed the test.

Credibility Factor: `r Posterior_PG$credibility_factor`

**Undergraduate Students:**
Estimator: `r E_UG`
Based on the above estimate of the posterior expected squared loss, we can say that 70% of the proportion of the postgraduate students passed the test.

Credibility Factor: `r Posterior_UG$credibility_factor`
...
In Bayes' Theorem, the absolute loss is defined as the median of the posterior distribution. Based on this, the estimator that minimises the expected absolute error loss within the posterior distribution for the proportion of students who passed is as follows:

 - **Postgraduate students:** `r E_PG_AE`
 - **Undergraduate students:** `r E_UG_AE`
...
Upon analysing the posterior distribution of the proportion of students who passed the quiz, several interesting features emerge. In the case of the undergraduate students, the posterior distribution exhibits a relatively normal shape. However, for the postgraduate students who passed the quiz, their posterior distribution displays a slight right skew.

Furthermore, it is evident that the mean score for postgraduate students is higher than that of the undergraduate students, as we can directly compare in the plot.
...
If the lecturer wishes to do this again next semester, and she intends to use the same or a similar quiz, we recommend using the posterior distribution from this semester as the prior. In our scenario, this prior was a beta distribution. This approach adheres to Bayesian principles by incorporating the latest data (scores) to update beliefs (the distribution of scores).

We would only utilise a uniform distribution as the prior in a new setting, such as a different form of testing (e.g., an assignment). In this case, we can assume an unbiased perspective due to the lack of prior information. This can be a valid alternative when the prior data is not relevant.
...
# Posterior mean and variance of the difference
...
The simulated difference in the proportions of postgraduate and undergraduate students has a mean of 0.139 and a variance of 0.004. This implies that, on average, postgraduate students may have scores that are approximately 13.9% higher, and this difference does not vary significantly, with a variance of only 0.4%.
