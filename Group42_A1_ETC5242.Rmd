...
In this case, we are interested in finding if PG students are actually better than UG students based on the quiz that they have taken. In doing so, we are going to perform upper one-sided hypothesis test, with $H_0$ and $H_1$ are:
$$H_0: p_{P}-p_{U} = 0 \quad \mbox{vs.} \quad H_1: p_{P}-p_{U} > 0,$$
where ${p}_{P}$ and ${p}_{U}$ denote the population proportions of Post-Graduate and Under-Graduate students passing the quiz, respectively. 

Based on the result that we have done previously, we have got p-value of the test is 0.0292. This suggest that based on the evidence we have, we reject the $H_0$. It means that it is likely there is a difference in the capability of PG students to pass the quiz compare to UG students, with PG students are better. To support this, we can see visually from the plot, our observed difference (the red line) is not likely to have come from a distribution which assumes that there are no difference between PG students and UG students, since it is far from 0.
...
There is a 95% probability that the true number of postgraduate students who passed falls within the range of 17.75 to 44.79.

There is a 95% probability that the true number of undergraduate students who passed falls within the range of 53.62 to 135.30.
...
```{r}
# PG STUDENTS
## Posterior and Credibility Factor Calculation
xPG <- 41
Posterior_PG <- beta_binomial(n = nPG, x = xPG, alpha = alpha, beta = beta)
Posterior_PG

## Estimator that minimises the posterior expected squared loss for PG
E_PG <- Posterior_PG$alpha_tilde / Posterior_PG$beta_tilde
E_PG
...
## Posterior and Credibility Factor Calculation
xUG <- 102
Posterior_UG <- beta_binomial(n = nUG, x = xUG, alpha = alpha, beta = beta)
Posterior_UG

## Estimator that minimises the posterior expected squared loss for PG
E_UG <- Posterior_UG$alpha_tilde / Posterior_UG$beta_tilde
E_UG
```
...
```{r}
# PG STUDENTS
## Estimator that minimises the posterior expected absolute loss for PG
