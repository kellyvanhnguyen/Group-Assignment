...
--RACHITHA--
Even though the Data continuous. The graph shows that it is skewed hence, beta distribution will be more appropriate. 
...
```{r norm-qqplot}
qq_norm <- ggplot(df, aes(sample = x)) +
  stat_qq(distribution = qnorm, dparams = params) +
...
MLE.x_boot <- matrix(rep(NA, 2 * B), nrow = B, ncol = 2)
for (i in 1:B) {
  tempK <- sample(grades$Total, size = n, replace = TRUE)
  scores <- df %>% mutate(temp = tempK)
  MLE.x_boot[i, ] <- fitdistr(temp, "normal")$estimate
  params_boot <- MLE.x_boot[i, ]
  qq_norm <-
    qq_norm + stat_qq(
...
qq_beta <- ggplot(dt, aes(sample = x)) +
...
grades %>% ggplot(aes(x=x, y=after_stat(density))) +
geom_histogram(colour="blue", fill="blue", alpha=0.2) +
